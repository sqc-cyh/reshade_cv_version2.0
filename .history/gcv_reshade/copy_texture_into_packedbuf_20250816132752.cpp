	...
	TextureInterpretation tex_interp, const depth_tex_settings &depth_settings)
{
	...
	else
	{
        ...
        // Determine the correct initial resource state before transitioning to copy source.
        // A color buffer is a render target, while a depth buffer is a depth stencil.
        reshade::api::resource_usage initial_state = reshade::api::resource_usage::undefined;
        if ((desc.usage & reshade::api::resource_usage::render_target) == reshade::api::resource_usage::render_target)
        {
            initial_state = reshade::api::resource_usage::render_target;
        }
        else if ((desc.usage & reshade::api::resource_usage::depth_stencil) == reshade::api::resource_usage::depth_stencil)
        {
            initial_state = reshade::api::resource_usage::depth_stencil;
        }

        // Only perform the transition if we identified a valid starting state.
        if (initial_state != reshade::api::resource_usage::undefined)
            cmd_list->barrier(tex, initial_state, reshade::api::resource_usage::copy_source);

        cmd_list->copy_texture_region(tex, 0, nullptr, intermediate, 0, nullptr);

        // Transition back to the original state after copying.
        if (initial_state != reshade::api::resource_usage::undefined)
            cmd_list->barrier(tex, reshade::api::resource_usage::copy_source, initial_state);
    }
