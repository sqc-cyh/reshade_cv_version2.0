	...
	TextureInterpretation tex_interp, const depth_tex_settings &depth_settings)
{
    ...
    {
        // This check is too restrictive for some games like Cyberpunk 2077,
        // where the depth buffer is not explicitly created with the copy_source flag.
        // We rely on a resource barrier to transition it to a copyable state instead.
        /*
        ...
        */
        ...
        // For DX12, the resource state must be transitioned correctly.
        // When capturing at present time, the depth buffer is typically in a read-only state.
        const resource_usage source_state = (tex_interp == TextureInterpretation::Depth) ? resource_usage::depth_stencil_read : resource_usage::render_target;

        // Transition the resource from its current state to a copy source
        cmd_list->barrier(tex, source_state, resource_usage::copy_source);

        // Perform the copy to the intermediate resource (which is on CPU-accessible memory)
        cmd_list->copy_texture_region(tex, 0, nullptr, intermediate, 0, nullptr);

		std::stringstream ss;
        ss << "Attempting copy. Interpretation: " << static_cast<int>(tex_interp)
           << ", Original Usage Flags: " << static_cast<uint32_t>(desc.usage)
           << ", Assumed Source State: " << static_cast<uint32_t>(source_state);
        reshade::log_message(reshade::log_level::info, ss.str().c_str());
        // Transition the resource back to its original state so the game can continue using it
        cmd_list->barrier(tex, resource_usage::copy_source, source_state);
    }
