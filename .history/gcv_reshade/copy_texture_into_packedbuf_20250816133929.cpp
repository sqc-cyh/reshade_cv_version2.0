	...
	TextureInterpretation tex_interp, const depth_tex_settings &depth_settings)
{
    ...
    {
        // This check is often too restrictive. We will rely on resource barriers instead.
        ...
        // Be explicit about the resource state. For depth buffers at present time,
        // it's almost always in a read-only state. For color buffers, it's a render target.
        const resource_usage source_state = (tex_interp == TextureInterpretation::Depth) ?
            resource_usage::depth_stencil_read :
            resource_usage::render_target;

        // Transition from the precise source state to copy source
