...
void depth_gray_bytesLE_to_f32(simple_packed_buf &dstBuf, const resource_desc &desc, const subresource_data &data,
	...
	if (settings.alreadyfloat) {
		if (!((srcpixbytes == sizeof(float) && depthbytes2keep == sizeof(float)) ||
          (srcpixbytes == 8 && depthbytes2keep == sizeof(float)))) {
        	reshade::log_message(reshade::log_level::error,
            "ERROR: settings.alreadyfloat() but invalid bytes per pix calculations");
			return;
		}
	}
	...
	for (y = 0; y < desc.texture.height; ++y, src_p += rowpitch) {
		...
		if (gamehandle_can_interpret_depth) {
			if (dstfp == nullptr) continue;
		} else {
			if (dstup == nullptr) continue;
		}
		if (!settings.debug_mode) {
			if (!settings.alreadyfloat) {
				...
				if (srcpixbytes == 8 && depthbytes2keep == 4) {
					// 低 4 字节即 D32 的 float depth
					for (x = 0; x < desc.texture.width; ++x) {
						const uint8_t *const src = src_p + x * srcpixbytes;
						float val;
						// 直接读低 4B；避免未对齐，用 memcpy 更稳妥
						std::memcpy(&val, src, 4);
						dstfp[x] = val;
					}
				}
				else if (settings.float_reverse_endian) {
					for (x = 0; x < desc.texture.width; ++x) {
						const uint8_t *const src = src_p + x * srcpixbytes;
						endianflip[3] = src[0];
						endianflip[2] = src[1];
	...
	TextureInterpretation tex_interp, const depth_tex_settings &depth_settings)
{
	...
	const format src_orig_fmt = desc.texture.format; // 保存源资源原始格式
	...
	else
	{
		...
		reshade::api::format dstfmt = desc.texture.format; // 尽量原样
		if (dstfmt == reshade::api::format::r32_g8_typeless ||
            dstfmt == reshade::api::format::r24_g8_typeless) {
            dstfmt = format_to_default_typed(desc.texture.format); // e.g. r32_float_x8_uint
        }
        desc.texture.format = dstfmt;
        ...
        //     desc.texture.format == format::r24_g8_typeless ||
        //     desc.texture.format == format::r32_g8_typeless ||
        //     desc.texture.format == format::r32_float_x8_uint;
		const bool is_depth_stencil =
+       src_orig_fmt == format::r24_g8_typeless ||
+       src_orig_fmt == format::r32_g8_typeless ||
+       src_orig_fmt == format::r32_float_x8_uint;
        const resource_usage from_state = is_depth_stencil ? resource_usage::depth_stencil
                                                           : resource_usage::shader_resource;
        cmd_list->barrier(tex, from_state, resource_usage::copy_source);
		...
		cmd_list->barrier(tex, resource_usage::copy_source, from_state);
	}
