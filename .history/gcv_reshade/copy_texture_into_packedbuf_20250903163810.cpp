...
void depth_gray_bytesLE_to_f32(simple_packed_buf &dstBuf, const resource_desc &desc, const subresource_data &data,
	...
	for (y = 0; y < desc.texture.height; ++y, src_p += rowpitch) {
		...
		if (!settings.debug_mode) {
			if (!settings.alreadyfloat) {
				// reshade::log_message(reshade::log_level::info, "copy_texture: interpreting depth buffer as integer");
				for (x = 0; x < desc.texture.width; ++x) {
					...
					if (gamehandle_can_interpret_depth) {
						...
						reshade::log_message(reshade::log_level::info, "copy_texture: interpreting depth buffer as uint32");
						dstup[x] = static_cast<uint32_t>(std::min(clipu32,vi));
					}
				}
			} else {
				if (settings.float_reverse_endian) {
					// reshade::log_message(reshade::log_level::info, "copy_texture: interpreting depth buffer as float with endian flip");
					for (x = 0; x < desc.texture.width; ++x) {
						const uint8_t *const src = src_p + x * srcpixbytes;
						endianflip[3] = src[0];
						endianflip[2] = src[1];
					...
					// reshade::log_message(reshade::log_level::info, "copy_texture: interpreting depth buffer as float");
					// src_f = reinterpret_cast<float *>(src_p);
					// for (x = 0; x < desc.texture.width; ++x) {
					// 	dstfp[x] = src_f[x];
					// }
					for (x = 0; x < desc.texture.width; ++x) {
						// srcpixbytes 对于 D32S8 应该是 8
						const uint8_t* src = src_p + x * srcpixbytes;
						// 将源指针的前 4 个字节重新解释为 float 指针并解引用
						dstfp[x] = *reinterpret_cast<const float*>(src);
					}
				}
			}
		} else {
	...
	TextureInterpretation tex_interp, const depth_tex_settings& depth_settings)
{
	...
	case format::r32_float_x8_uint:
		if (tex_interp != TexInterp_Depth || !dstBuf.set_pixfmt_and_alloc_bytes(BUF_PIX_FMT_GRAYF32)) {
			reshade::log_message(reshade::log_level::error, "Failed: tex_interp != TexInterp_Depth or set_pixfmt_and_alloc_bytes failed");
			return false;
		}
		reshade::log_message(reshade::log_level::info, "Passed if, continue processing");
