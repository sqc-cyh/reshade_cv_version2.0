                      ...
                      const DepthToneParams& p)
{
  ...
  switch (pbuf.pixfmt) {
    ...
    case BUF_PIX_FMT_GRAYU32: {
      reshade::log_message(reshade::log_level::info, "grabbers: enter GRAYU32 branch");
      ...
      const bool invert = true;  // 设为 true: 近黑远白；false: 近白远黑
      const float lo = p.clip_low;
      const float hi = (p.clip_high > lo ? p.clip_high : lo + 1e-6f);
      const float invspan = 1.0f / (hi - lo);
      ...
      for (int y = 0; y < h; ++y) {
        ...
        uint8_t* dst = out_gray.data() + (size_t)y * (size_t)w;
        for (int x = 0; x < w; ++x) {
          double t0 = (double(src[x]) - double(vmin)) / span;  // vmin→0, vmax→1
          float  t = float(t0);
          if (invert) t = 1.0f - t;                            // 反转极性：近黑远白
          t = (t - lo) * invspan;
          if (t < 0.f) t = 0.f; else if (t > 1.f) t = 1.f;
          uint8_t g = map01_farwhite_log(t);                   // 0..255
          dst[x] = g;
        }
      }
      return true;
...
      const double span = (vmax > vmin) ? double(vmax - vmin) : 1.0;
      ...
      // 动态判断是否需要反转深度值，以兼容反向Z-Buffer
      const bool needs_inversion = (vmin > vmax); 

      for (int y = 0; y < h; ++y) {
        const uint32_t* src = pbuf.rowptr<uint32_t>(y);
        uint8_t* dst = out_gray.data() + (size_t)y * (size_t)w;
        for (int x = 0; x < w; ++x) {
          double t = (double(src[x]) - double(vmin)) / span; // vmin→0, vmax→1
          if (needs_inversion) {
            t = 1.0 - t; // 反转极性
          }
          if (t < 0.f) t = 0.f; else if (t > 1.f) t = 1.f;
          uint8_t g = map01_farwhite_log(static_cast<float>(t));
          dst[x] = g;
        }
      }
      return true;
