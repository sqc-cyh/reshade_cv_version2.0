...
struct FfmpegPipe {
    ...
    bool start(int width, int height, int fps_, const std::string& outdir) {
		...
		if (!CreatePipe(&hRead, &hWrite, &sa, 1<<20)) return false;        // 1MB 缓冲
		if (!SetHandleInformation(hWrite, HANDLE_FLAG_INHERIT, 0)) { CloseHandle(hRead); CloseHandle(hWrite); return false; }
		...
		auto make_cmd = [&](bool nvenc){
			std::ostringstream cmd;
			cmd << "ffmpeg -loglevel error -y "
				<< "-f rawvideo -pix_fmt bgra -s " << w << "x" << h << " -r " << fps << " -i pipe:0 ";
			if (nvenc) cmd << "-c:v h264_nvenc -preset p5 -rc vbr -b:v 10M -maxrate 20M ";
			else       cmd << "-c:v libx264 -preset veryfast -crf 18 ";
			cmd << "-pix_fmt yuv420p -movflags +faststart \"" << out_mp4 << "\"";
			return cmd.str();
		};

		std::string cmdline = make_cmd(true);
		BOOL ok = CreateProcessA(nullptr, cmdline.data(), nullptr, nullptr, TRUE, CREATE_NO_WINDOW, nullptr, nullptr, &si, &pi);
		if (!ok) { cmdline = make_cmd(false);
				ok = CreateProcessA(nullptr, cmdline.data(), nullptr, nullptr, TRUE, CREATE_NO_WINDOW, nullptr, nullptr, &si, &pi); }
		// 父进程不再需要读端：立刻关闭（非常关键）
		CloseHandle(hRead);
...
static void pipe_writer_loop() {
    ...
    while (g_pipe_thread_run.load(std::memory_order_acquire)) {
        ...
        if (f.data && f.size && g_ff.hProc && g_ff.hStdinWrite) {
            DWORD wrote = 0;
            if (!WriteFile(g_ff.hStdinWrite, f.data.get(), (DWORD)f.size, &wrote, nullptr)) {
                                              ...
                                              std::string& cmdline_fallback)
{
    ...
    if (tried_nvenc && !cmdline_fallback.empty()) {
        reshade::log_message(reshade::log_level::warn, "[CV Capture] ffmpeg (nvenc) exited immediately; try x264 fallback");
