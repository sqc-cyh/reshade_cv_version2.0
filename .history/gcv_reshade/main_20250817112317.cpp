...
struct FfmpegPipe {
    ...
    bool start(const std::string &cmdline, const std::string& out_dir_raw) {
		...
		for (auto &ch : out_dir) if (ch == '/') ch = '\\';   // 统一成反斜杠
	...
	bool start(int width, int height, int fps, const std::string& outdir) {
		// 先试 NVENC，失败再试 x264
		std::ostringstream cmd_nvenc;
		cmd_nvenc << "ffmpeg -loglevel error -y "
				<< "-f rawvideo -pix_fmt bgra -s " << width << "x" << height
				<< " -r " << fps << " -i pipe:0 "
				<< "-c:v h264_nvenc -preset p5 -rc vbr -b:v 10M -maxrate 20M "
				<< "-pix_fmt yuv420p -movflags +faststart "
				<< "\"" << (outdir + "capture.mp4") << "\"";

		if (start(cmd_nvenc.str()))            // 调用已有的 start(cmdline)
			return true;
