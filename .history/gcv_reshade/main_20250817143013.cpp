...
{
    simple_packed_buf pbuf;
    depth_tex_settings depth_cfg{}; // 使用默认配置；若需要反转/线性化可在这里加开关
    if (!copy_texture_image_needing_resource_barrier_into_packedbuf(
    ...
    {
        // 约定：0 近、1 远（如果你想反过来，下面把 d 改成 1.0f - d）
        for (int y = 0; y < h; ++y) {
            const float* src = pbuf.rowptr<float>(y);
            uint8_t* dst = out_gray.data() + (size_t)y * (size_t)w;
            for (int x = 0; x < w; ++x) {
                float d = src[x];
                if (!(d == d)) d = 1.0f;           // NaN 保护
                if (d < 0.f) d = 0.f; 
                if (d > 1.f) d = 1.f;
                // 如需反转：d = 1.0f - d;
                dst[x] = (uint8_t)std::lround(d * 255.0f);
            }
        }
        return true;
    }
    ...
    {
        // 原值是 0..(max) 的 32 位无符号整，做每帧 min-max 归一化到 0..255
        ...
        for (int y = 0; y < h; ++y) {
            ...
            uint8_t* dst = out_gray.data() + (size_t)y * (size_t)w;
            for (int x = 0; x < w; ++x) {
                double t = (double(src[x]) - double(vmin)) / span; // 0..1
                if (t < 0.0) t = 0.0; if (t > 1.0) t = 1.0;
                // 如需反转：t = 1.0 - t;
                dst[x] = (uint8_t)std::lround(t * 255.0);
            }
        }
        return true;
    }
