...
// 录制状态
static std::unique_ptr<GameInterface> g_game; 
...
static void on_init(reshade::api::device* device)
{
    ...
    reshade::log_message(reshade::log_level::info, std::string("tests: ")+run_utils_tests());
    shdata.init_time = hiresclock::now();

    // 关键：创建当前进程对应的 GameInterface（factory 里已注册了 CP2077）
    g_game = gcv_games::make_game_interface_for_current_process(); // 常见名称，与你项目里的实际工厂函数名一致即可
    if (!g_game) {
        reshade::log_message(reshade::log_level::warning, "[CV Capture] no game interface (depth will be raw).");
    } else {
        reshade::log_message(reshade::log_level::info, "[CV Capture] game interface ready.");
    }
}
struct FfmpegPipe {
