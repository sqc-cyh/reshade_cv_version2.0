...
// -----------------------------------control信号---------------------------
// 位: 0:W, 1:A, 2:S, 3:D, 4:Shift, 5:Space
enum : uint32_t {
    KM_W = 1u << 0,
    KM_A = 1u << 1,
    KM_S = 1u << 2,
...
// 直接覆盖像素（BGRA）
static inline void set_px_bgra(uint8_t* img, int W, int H, int x, int y,
                               uint8_t b, uint8_t g, uint8_t r, uint8_t a=255)
{
    if (x < 0 || y < 0 || x >= W || y >= H) return;
    size_t idx = ((size_t)y * (size_t)W + (size_t)x) * 4;
    img[idx+0] = b; img[idx+1] = g; img[idx+2] = r; img[idx+3] = a;
}

// 简单 alpha 叠加到已有像素（前景 src 覆盖到 dst）
static inline void blend_px_bgra(uint8_t* img, int W, int H, int x, int y,
                                 uint8_t b, uint8_t g, uint8_t r, uint8_t a)
{
    ...
    img[idx+0] = u8clamp((int)(af * b + ia * img[idx+0]));
    img[idx+1] = u8clamp((int)(af * g + ia * img[idx+1]));
    img[idx+2] = u8clamp((int)(af * r + ia * img[idx+2]));
    // 保留原 alpha
}

static void fill_rect(uint8_t* img, int W, int H, int x0, int y0, int w, int h,
                      uint8_t b, uint8_t g, uint8_t r, uint8_t a=255)
{
    int x1 = x0 + w, y1 = y0 + h;
    for (int y = y0; y < y1; ++y)
        for (int x = x0; x < x1; ++x)
            blend_px_bgra(img, W, H, x, y, b, g, r, a);
}

static void rect_outline(uint8_t* img, int W, int H, int x0, int y0, int w, int h,
                         uint8_t b, uint8_t g, uint8_t r, int thick=1, uint8_t a=255)
{
    for (int t=0; t<thick; ++t) {
        int x1 = x0 + w - 1 - t, y1 = y0 + h - 1 - t;
        for (int x = x0 + t; x <= x1; ++x) {
            blend_px_bgra(img, W, H, x, y0 + t, b,g,r,a);
            blend_px_bgra(img, W, H, x, y1,   b,g,r,a);
        }
        for (int y = y0 + t; y <= y1; ++y) {
            blend_px_bgra(img, W, H, x0 + t, y, b,g,r,a);
            blend_px_bgra(img, W, H, x1,     y, b,g,r,a);
        }
    }
}

// 在左下角画键位 HUD
static void draw_keys_hud(uint8_t* img, int W, int H, uint32_t keymask)
{
    // 尺寸/布局（随分辨率自适应一点点）
    const int pad   = std::max(8, W/200);     // 边距
    const int box   = std::max(26, W/80);     // 按键方块尺寸
    const int gap   = std::max(6,  W/320);    // 间距
    const int thick = std::max(2,  W/640);    // 边框粗细

    const int hudW = box*3 + gap*2;
    const int hudH = box*2 + gap*2 + box/2 + gap + box/2; // 2行 + shift 条 + space 条

    int originX = pad;
    int originY = H - pad - hudH;

    // 背板
    fill_rect(img, W, H, originX-4, originY-4, hudW+8, hudH+8, 0,0,0, 120);

    auto draw_key = [&](int x, int y, bool pressed) {
        uint8_t fr=255, fg=255, fb=255;   // 前景
        uint8_t br=40,  bg=40,  bb=40;    // 背景
        uint8_t pr=60,  pg=180, pb=255;   // 按下色
        fill_rect(img,W,H,x,y,box,box, pressed?pb:bg, pressed?pg:bg, pressed?pr:br, 200);
        rect_outline(img,W,H,x,y,box,box, 255,255,255, thick, 200);
    };

    // WASD 排布：W 在上中；A/S/D 在下一行
    int xW = originX + box + gap;     int yW = originY;
    int xA = originX;                 int yA = originY + box + gap;
    int xS = originX + box + gap;     int yS = yA;
    int xD = originX + (box+gap)*2;   int yD = yA;

    draw_key(xW,yW, (keymask & KM_W)!=0);
    draw_key(xA,yA, (keymask & KM_A)!=0);
    draw_key(xS,yS, (keymask & KM_S)!=0);
    draw_key(xD,yD, (keymask & KM_D)!=0);

    // Shift 条（长条）
    int shY = yA + box + gap;
    int shH = box/2;
    int shW = hudW;
    fill_rect(img,W,H, originX, shY, shW, shH,
              (keymask&KM_SHIFT)?255:40, (keymask&KM_SHIFT)?180:40, (keymask&KM_SHIFT)?60:40, 200);
    rect_outline(img,W,H, originX, shY, shW, shH, 255,255,255, thick, 200);

    // Space 条（更长/或同长）
    int spY = shY + shH + gap;
    int spH = box/2;
    int spW = hudW;
    fill_rect(img,W,H, originX, spY, spW, spH,
              (keymask&KM_SPACE)?255:40, (keymask&KM_SPACE)?180:40, (keymask&KM_SPACE)?60:40, 200);
    rect_outline(img,W,H, originX, spY, spW, spH, 255,255,255, thick, 200);
}
// ---------------------------------------------------------------
...
static inline void blend_px_gray(uint8_t* img, int W, int H, int x, int y,
                                 uint8_t val, uint8_t a /*0..255*/)
{
    if (x < 0 || y < 0 || x >= W || y >= H) return;
    size_t idx = (size_t)y * (size_t)W + (size_t)x;
    const float af = a / 255.0f, ia = 1.0f - af;
    img[idx] = u8clamp((int)(af * val + ia * img[idx]));
}

static void fill_rect_gray(uint8_t* img, int W, int H, int x0, int y0, int w, int h,
                           uint8_t val, uint8_t a = 255)
{
    int x1 = x0 + w, y1 = y0 + h;
    for (int y = y0; y < y1; ++y)
        for (int x = x0; x < x1; ++x)
            blend_px_gray(img, W, H, x, y, val, a);
}

static void rect_outline_gray(uint8_t* img, int W, int H, int x0, int y0, int w, int h,
                              uint8_t val, int thick = 1, uint8_t a = 255)
{
    for (int t=0; t<thick; ++t) {
        int x1 = x0 + w - 1 - t, y1 = y0 + h - 1 - t;
        for (int x = x0 + t; x <= x1; ++x) {
            blend_px_gray(img,W,H,x, y0 + t, val, a);
            blend_px_gray(img,W,H,x, y1,     val, a);
        }
        for (int y = y0 + t; y <= y1; ++y) {
            blend_px_gray(img,W,H,x0 + t, y, val, a);
            blend_px_gray(img,W,H,x1,     y, val, a);
        }
    }
}

// 灰度 HUD：按下亮，未按暗；留白色描边
static void draw_keys_hud_gray(uint8_t* img, int W, int H, uint32_t keymask)
{
    const int pad   = std::max(8, W/200);
    const int box   = std::max(26, W/80);
    const int gap   = std::max(6,  W/320);
    const int thick = std::max(2,  W/640);
    ...
    // 背板（半透明暗）
    fill_rect_gray(img, W, H, originX-4, originY-4, hudW+8, hudH+8, 0, 120);

    auto draw_key = [&](int x, int y, bool pressed) {
        uint8_t v_bg = 40;    // 未按
        uint8_t v_on = 220;   // 按下
        fill_rect_gray(img,W,H,x,y,box,box, pressed?v_on:v_bg, 200);
    ...
    int shH = box/2;
    int shW = hudW;
    fill_rect_gray(img,W,H, originX, shY, shW, shH, (keymask&KM_SHIFT)?220:40, 200);
    rect_outline_gray(img,W,H, originX, shY, shW, shH, 255, thick, 200);

    // Space 条
    int spY = shY + shH + gap;
    int spH = box/2;
    int spW = hudW;
    fill_rect_gray(img,W,H, originX, spY, spW, spH, (keymask&KM_SPACE)?220:40, 200);
    rect_outline_gray(img,W,H, originX, spY, spW, spH, 255, thick, 200);
}
// ----------------------------------------------------------------------------
			...
			if (g_last_cap_us == 0 || (now_us - g_last_cap_us) >= min_interval_us) {
				...
				if (color_res.handle == 0) {
					...
					if (grab_bgra_frame(q, color_res, bgra, w, h)) {
						g_copy_fail_in_row = 0;
						draw_keys_hud(bgra.data(), w, h, keymask);
