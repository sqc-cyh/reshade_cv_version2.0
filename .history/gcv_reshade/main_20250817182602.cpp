...
// 				fprintf(g_actions_csv, "frame_idx,time_us,w,a,s,d,shift,space\n");
// 			}
...
static std::string g_rec_dir;
static FILE* g_actions_csv = nullptr; // 仅用于判断是否需要写表头；真正写在 Recorder
static uint64_t g_rec_idx = 0;
static int64_t g_last_cap_us = 0;
static int g_copy_fail_in_row = 0;
static const int g_copy_fail_stop_threshold = 60;
static DepthToneParams g_depth_tone; // clip/log 参数

// ------------------ 原始 on_init / on_destroy 保留 ------------------
static void on_init(reshade::api::device* device)
{
    auto &shdata = device->create_private_data<image_writer_thread_pool>();
    reshade::log_message(reshade::log_level::info, std::string(std::string("tests: ")+run_utils_tests()).c_str());
    ...
    reshade::api::command_list *, reshade::api::resource_view rtv, reshade::api::resource_view)
{
    ...
    // ====== 以下保留你原有的 F11 单帧抓取与 UI/相机坐标更新逻辑 ======
    if (segmentation_app_update_on_finish_effects(runtime, runtime->is_key_pressed(VK_F11))) {
        // ... （原样粘贴你 F11 逻辑，不改）
        // 这里无需触动 grab/ffmpeg/hud
        // （略，为节省篇幅，按你现有 main.cpp 该段保留）
    }

    // 其余 camcoords 与 overlay 更新逻辑保持不变（略）
    // ...
}

// 你的 draw_settings_overlay 保持不变；可选把深度 tone 参数暴露到 ImGui：
// ImGui::SliderFloat("Depth clip low",  &g_depth_tone.clip_low,  0.f, 1.f);
// ImGui::SliderFloat("Depth clip high", &g_depth_tone.clip_high, 0.f, 1.f);
// ImGui::SliderFloat("Depth log alpha",&g_depth_tone.log_alpha, 0.f, 12.f);

// DllMain 与事件注册保持不变（原样）
