            ...
            if (now_us >= next_due_us) {
                ...
                if (color_res.handle == 0) {
                    ...
                    if (grab_bgra_frame(q, color_res, bgra, w, h)) {
                        ...
                        Json camj;
                        if (cam_ok) {
                            cam.into_json(camj);     // 字段与单帧 meta.json 保持一致
                        } else {
                            camj["cam_status"] = "uninitialized";
                        	if (!cam_err.empty()) camj["err"] = cam_err;
                        }
                        // 与本帧颜色/动作同一 frame_idx 与时间戳
                        const uint64_t start_idx = g_rec_idx;
                        g_rec->log_camera_json(/*idx=*/start_idx,
                                               /*time_us=*/now_us,
                                               /*cam_json=*/camj,
                                               /*img_w=*/w, /*img_h=*/h);
                        g_rec->log_action(start_idx, now_us, keymask);

                        // 为插值帧补写相机行（k=1..need-1），时间戳按 CFR 等间隔推进
                        for (int k = 1; k < need; ++k) {
                            g_rec->log_camera_json(/*idx=*/start_idx + k,
                                                   /*time_us=*/base_due + k * period_us,
                                                   /*cam_json=*/camj,
                                                   /*img_w=*/w, /*img_h=*/h);
                        }

                        // 生成插值帧并一次性补齐索引：本周期总共 +need
                        g_rec->duplicate(need - 1);
                        g_rec_idx += need;

                        // 更新“上一帧相机缓存”，以便下次失败时用于补写
                        s_last_camj = camj; s_has_camj = true;
                    } else {
                        g_copy_fail_in_row++;
						...
						if (s_has_camj && need > 1) {
+                        const uint64_t start_idx = g_rec_idx; // 本周期没有写真实帧，所以从当前 idx+1 起写 need-1 行
+                        for (int k = 1; k < need; ++k) {
+                            g_rec->log_camera_json(/*idx=*/start_idx + (k - 0),     // 从 start_idx 开始连续写
+                                                   /*time_us=*/base_due + k * period_us,
+                                                   /*cam_json=*/s_last_camj,
+                                                   /*img_w=*/0, /*img_h=*/0);     // 无法确定分辨率时可写 0
+                        }
+                        // 仅补插值帧数量（need-1）
+                        g_rec_idx += (need - 1);
+                    }
