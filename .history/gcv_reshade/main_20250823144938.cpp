...
#include "glassmask/glassmask.hpp"
...
static GlassMaskSystem* g_glassmask = nullptr;
...
static void on_init(reshade::api::device* device)
{
    ...
    shdata.init_time = hiresclock::now();
	 ...
	 // glassmask 初始化
    constexpr uint32_t GLASS_HEX = 0x9b2edb;
    GlassClassifierConfig cfg;
    cfg.seg_hex_colors = { GLASS_HEX };
    cfg.use_heuristic = false;
    g_glassmask = new GlassMaskSystem(device, cfg);
    g_glassmask->install();
}
				...
				if (color_res.handle == 0) {
					...
					if (grab_bgra_frame(q, color_res, bgra, w, h)) {
                        ...
                        if (depth_res.handle != 0) {
                            ...
                            if (shdata.game_knows_depthbuffer()) {               // 额外生成 depth.fpzip
