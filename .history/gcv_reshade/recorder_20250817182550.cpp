...
bool Recorder::start(){
  ...
  // 打开 CSV（无缓冲）
  if (cfg_.write_csv) {
    std::string csv = cfg_.out_dir;
    if (!csv.empty() && csv.back()!='/' && csv.back()!='\\') csv.push_back('/');
    csv += "actions.csv";
    csv_ = _fsopen(csv.c_str(), "w", _SH_DENYNO);
    if (csv_) { setvbuf(csv_, nullptr, _IONBF, 0); std::fprintf(csv_, "frame_idx,time_us,w,a,s,d,shift,space\n"); }
    else reshade::log_message(reshade::log_level::error, "[CV Capture] open actions.csv failed");
  }
  return true;
}
...
void Recorder::push_color(const uint8_t* bgra,int w,int h){
  ...
  if (!q_push(ring_c_, prod_c_, cons_c_, std::move(f), cap_c_)) { /* 丢帧 */ }
...
void Recorder::push_depth(const uint8_t* gray,int w,int h){
  ...
  if (!q_push(ring_d_, prod_d_, cons_d_, std::move(f), cap_d_)) { /* 丢帧 */ }
...
void Recorder::duplicate(int n){
  ...
  for (int i=0;i<n;++i){
    if (pipe_c_.alive() && lw_>0 && lh_>0){
      ...
      q_push(ring_c_, prod_c_, cons_c_, std::move(f), cap_c_);
    }
    if (pipe_d_.alive() && dw_>0 && dh_>0){
      ...
      q_push(ring_d_, prod_d_, cons_d_, std::move(f), cap_d_);
    }
  }
}
...
void Recorder::color_loop(){
  ...
  while (th_run_c_.load(std::memory_order_acquire)){
    ...
    if (f.data && f.size && pipe_c_.alive() && pipe_c_.hWrite()){
      ...
      } else { written_.fetch_add(1, std::memory_order_relaxed); }
    }
    f = {};
  }
}
