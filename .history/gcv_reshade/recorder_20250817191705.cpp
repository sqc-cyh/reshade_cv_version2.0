...
bool Recorder::start(){
  ...
  // 1) 打开 CSV（关键：无缓冲，崩溃也尽量不丢）
  if (cfg_.write_csv) {
    const std::string csv_path = join_path_slash(cfg_.out_dir) + "actions.csv";
    ...
    if (csv_) {
      ...
      reshade::log_message(reshade::log_level::error, "[CV Capture] open actions.csv failed");
    }
  }

  // 视频在首帧到来时再启动（分辨率未知时不启动）
  return true;
}
