...
+{
+  if (!running_) return;
+  if (!cam_jsonl_.is_open()) return;
+  try {
+    nlohmann::json j = cam_json;   // 复用 into_json 字段
+    j["frame_idx"] = idx;
+    j["time_us"]   = t_us;
+    j["img_w"]     = img_w;
+    j["img_h"]     = img_h;
...
+    cam_jsonl_.flush();
+  } catch (...) {
+    reshade::log_message(reshade::log_level::error, "[CV Capture] exception writing cam.jsonl");
+  }
+}