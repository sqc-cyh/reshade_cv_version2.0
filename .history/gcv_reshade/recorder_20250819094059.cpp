...
// 逐帧写入 cam.jsonl
// 逐帧写入 cam.jsonl + 每帧独立 camera.json
                               ...
                               int img_w, int img_h)
{
  ...
  try {
    // 1) 组装一份完整相机 JSON
    Json j = cam_json;        // 复用 into_json 的相机字段
    j["frame_idx"] = idx;
    j["time_us"]   = t_us;
    j["img_w"]     = img_w;
    j["img_h"]     = img_h;

    // 2) 仍写 cam.jsonl（保持原行为，便于串流/审计）
    if (cam_jsonl_.is_open()) {
      cam_jsonl_ << j.dump() << '\n';
      cam_jsonl_.flush();
    }

    // 3) 新增：每帧各写一份 camera.json（或 meta.json）
    //    路径形如: <out_dir>/frame_000123_camera.json
    const std::string out_dir_norm = join_path_slash(cfg_.out_dir);
    char namebuf[128];
