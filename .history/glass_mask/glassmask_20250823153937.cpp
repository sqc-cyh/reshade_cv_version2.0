...
static void on_bind_pipeline_cb(reshade::api::command_list* cmd,
                                reshade::api::pipeline_stage stages,
                                reshade::api::pipeline p) {
    if (g_sys) g_sys->on_bind_pipeline(cmd, stages, p);
    // 注意：bind_pipeline 事件是 void 回调，不应返回 bool
}

static bool on_draw_cb(reshade::api::command_list* cmd,
                       uint32_t vertex_count, uint32_t instance_count,
                       uint32_t first_vertex, uint32_t first_instance) {
    if (g_sys && g_sys->should_skip_draw(cmd, vertex_count)) return true;
    return false;
}

static bool on_draw_indexed_cb(reshade::api::command_list* cmd,
                               uint32_t index_count, uint32_t instance_count,
                               uint32_t first_index, int32_t vertex_offset,
                               uint32_t first_instance) {
    if (g_sys && g_sys->should_skip_draw(cmd, index_count)) return true;
    return false;
}
