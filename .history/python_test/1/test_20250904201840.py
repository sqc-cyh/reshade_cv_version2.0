...
def build_cv_c2w_from_ue(location, rotation):
    ...
    return R_cv, t_cv
def build_cv_c2w_from_gta_v1(location, rotation):
    # 1. 在 GTA 坐标系下构建原始的旋转矩阵
    R_gta = ue_rotator_to_R_world(rotation['roll'], rotation['pitch'], rotation['yaw'])
    
    # 2. 定义从 GTA 到 CV 的轴映射关系
    # GTA(+X=右, +Y=前, +Z=上) -> CV(+X=右, +Y=下, +Z=前)
    # CV_X =  GTA_X
    # CV_Y = -GTA_Z
    # CV_Z =  GTA_Y
    
    # 3. 对旋转矩阵的“列”进行重排和变号，以匹配新的坐标系
    # R_gta 的三列分别代表 GTA 的 X, Y, Z 轴在世界中的方向
    gta_x_axis, gta_y_axis, gta_z_axis = R_gta[:, 0], R_gta[:, 1], R_gta[:, 2]
    
    R_cv = np.stack([
        gta_x_axis,  # CV 的 X 轴就是 GTA 的 X 轴
        -gta_z_axis, # CV 的 Y 轴是 GTA 的 -Z 轴
        gta_y_axis   # CV 的 Z 轴是 GTA 的 Y 轴
    ], axis=1) # 按列堆叠

    # 4. 对平移向量进行同样的重排和变号
    t_cv = np.array([
        location['x'],  # CV 的 X 坐标是 GTA 的 X 坐标
        -location['z'], # CV 的 Y 坐标是 GTA 的 -Z 坐标
        location['y']   # CV 的 Z 坐标是 GTA 的 Y 坐标
    ], dtype=np.float64)
