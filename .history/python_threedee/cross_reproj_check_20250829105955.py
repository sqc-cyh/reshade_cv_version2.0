...
def backproject(depth, K, stride=4, zmin=1e-4, zmax=1e6):
    H, W = depth.shape
    ...
    z = depth[vv, uu]
    m = (z > zmin) & (z < zmax)
    uu, vv, z = uu[m].astype(np.float64), vv[m].astype(np.float64), z[m].astype(np.float64)
    x = (uu - cx) / fx * z
    y = (vv - cy) / fy * z
    Pc = np.stack([x, y, z, np.ones_like(z)], axis=0)  # 4xN
    return Pc, uu, vv, z  # 相机坐标 & 原像素坐标
...
def cross_check(rgbA, depthA, metaA, rgbB, depthB, metaB, stride=4, dz_rel_thr=0.05, dz_abs_thr=0.05):
    ...
    Pw  = Twc_A @ PcA          # world = inv(world->cam_A) * PcA
    PcB = Tcw_B @ Pw  
