...
def backproject(depth, K, stride=4, zmin=1e-4, zmax=1e6, depth_kind="Z"):
    ...
    return Pc, uu, vv, Z
def adapt_cam_axes(T_cw, axis_adapter):
    # 把“图像坐标系(y向下)”与“外参用的相机坐标系(y向上)”对齐
    S = np.eye(4)
    if axis_adapter in ("flip_y","flip_xy"):
        S[1,1] = -1.0   # 翻转相机Y
    if axis_adapter in ("flip_x","flip_xy"):
        S[0,0] = -1.0   # 如有需要再翻转X
    # 若 Pc_down 是你反投影得到的相机坐标(假设y向下)，而外参 T_cw_up 以y向上定义，
    # 则世界点 X_w = T_cw_up * (S * Pc_down) = (T_cw_up @ S) * Pc_down
    return T_cw @ S
...
def cross_check(rgbA, depthA, metaA, rgbB, depthB, metaB, stride=4, dz_rel_thr=0.05, dz_abs_thr=0.05):
    ...
    Tcw_A = adapt_cam_axes(Tcw_A, args.axis_adapter)
    Tcw_B = adapt_cam_axes(Tcw_B, args.axis_adapter)
    ...
    # A: 反投影（相机A坐标）
    PcA, uA, vA, zA = backproject(ZA, KA, stride=stride, depth_kind=args.depth_kind)
    PcA[0:3, :] *= args.m2w        # 关键：米 -> 世界坐标单位（与 t 的单位对齐）
    # A->世界->B相机
    Pw  = Tcw_A @ PcA
...
if __name__ == "__main__":
    ...
    # argparse 增加一个参数
    ap.add_argument("--axis_adapter", choices=["none","flip_y","flip_x","flip_xy"], default="none")
    ap.add_argument("--m2w", type=float, default=1.0, help="meters -> world units scale")
