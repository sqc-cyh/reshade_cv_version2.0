...
def fit_depth_data():
    ...
    try:
        ...
        popt, pcov = curve_fit(inverse_func, x_data, y_data, p0=initial_guess)
        A, B, C = popt
    except RuntimeError as e:
        print(f"\n曲线拟合失败: {e}")
        print("请尝试采集更多样化的数据点（特别是近距离和远距离）。")
        return

    # --- 6. 输出结果 ---
    print("\n" + "="*40)
    print("曲线拟合成功！")
    print(f"拟合出的参数 [A, B, C]: {popt}")
    print("="*40)
    ...
    print("    }")
    print("\n    // 根据曲线拟合得出的公式")
    print(f"    return {A:.8f}f / (depth + {B:.8f}f) - {C:.8f}f;")
    print("}")
    print("-" * 50)
    ...
    plt.plot(smooth_x, inverse_func(smooth_x, *popt), 'b-', label='拟合曲线')
