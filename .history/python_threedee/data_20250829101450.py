...
def robust_inverse_func(depth, A, B):
        return A / (depth + B)
    ...
    try:
        # 为稳健模型提供一个更好的初始猜测值 [A, B]
        # 从数据看，当 depth 约为 0.05 时，distance 约为 2。
        # A 约等于 distance * depth，所以 A 约为 0.1。B 应该很小。
        initial_guess = [0.1, 0.0]
        popt, pcov = curve_fit(robust_inverse_func, x_data, y_data, p0=initial_guess, maxfev=5000)
        A, B = popt
    ...
    # 检查参数是否合理（A和B都应该是正数）
    if A < 0:
        print("\n警告: 拟合参数 A 为负，结果可能不正确。尝试反转A和B的符号。")
        A = -A
        B = -B
        
    print("\n" + "="*40)
    print("曲线拟合成功！(使用稳健的反比例模型)")
    print(f"拟合出的参数 [A, B]: [{A}, {B}]")
    print("="*40)
    ...
    print("\n    // ... (check for small depth) ...")
    print("\n    // 根据曲线拟合得出的公式 (稳健模型)")
    print(f"    return {A:.8f}f / (depth + {B:.8f}f);")
    print("}")
    print("-" * 50)
    ...
    smooth_x = np.linspace(min(x_data), max(x_data), 200)
    plt.plot(smooth_x, robust_inverse_func(smooth_x, A, B), 'b-', label='拟合曲线 (稳健模型)')
    
    plt.title('GTA V 深度值与物理距离的拟合曲线')
    plt.xlabel('浮点深度值')
    plt.ylabel('物理距离 (米)')
    plt.legend()
    plt.grid(True)
