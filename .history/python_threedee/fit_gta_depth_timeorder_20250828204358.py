...
def main():
    ...
    print("N =", len(depths), " d[min,max,std] =", depths.min(), depths.max(), depths.std())
for i in range(min(10, len(depths))):
    print(i, files[i], "d=", depths[i], "z=", dists[i])
print("corr(1/z, d)   =", np.corrcoef(1.0/np.asarray(dists), np.asarray(depths))[0,1])
print("corr(1/z, 1-d) =", np.corrcoef(1.0/np.asarray(dists), 1.0-np.asarray(depths))[0,1])
