...
def load_cloud_via_depth_and_camjson(
        ...
        depth_is_euclidean:bool=False,   # <---- 新增开关
        ):
    ...
    if fov_degrees_vertical or fov_degrees_horizontal:
        fov_v, _ = fovv_and_fovh_degrees_given_either(fov_degrees_vertical,
                                                     fov_degrees_horizontal,
    ...
    if depth_is_euclidean:
        # ---- 新分支：射线方向欧氏距离 ----
        fx = 0.5 * screen_height / np.tan(0.5 * np.deg2rad(fov_v))
        fy = fx
        ...
        pts_cam = depth_image[..., None] * ray
        pts_cam = pts_cam.reshape(-1, 3)
        pts_cam_h = np.concatenate([pts_cam, np.ones((pts_cam.shape[0],1))], axis=1)
        wpoints = (cam2world @ pts_cam_h.T).T[:, :3]
        imcoords = np.stack([u.reshape(-1), v.reshape(-1)], axis=1)
        ...
        wpoints = np.ascontiguousarray(
            np.matmul(screen2world, wpoints).transpose()[:,:3]
        )
        if colored:
            rgb = rgb.reshape((-1,3))

    ret_ = {'worldpoints': wpoints, 'pixcoords': imcoords,
...
if __name__ == '__main__':
    ...
    parser.add_argument("--depth_is_euclidean", action="store_true",
                        help="Set this if depth.npy stores Euclidean ray distance (e.g., Witcher3, GTA5, Cyberpunk).")
