...
def load_cloud_via_depth_and_camjson(depthfile:str,
    ...
    # 正确构建4x4外参矩阵
    cam2world_3x4 = np.float64(camjson['extrinsic_cam2world']).reshape((3,4))
    cam2world = np.eye(4)
    cam2world[:3, :4] = cam2world_3x4
    
    cam2screen = build_intrinsicmatrix_camtoscreenpix_pinhole_camera(fov_vertical_degrees=fov_v, \
        screen_width=screen_width, screen_height=screen_height)
    
    # 计算 world2screen 和 screen2world 用于颜色映射
    ...
    # 直接将相机坐标系下的点变换到世界坐标系
    # 假设 wpoints.shape == (3, N)
    print("wpoints.shape =", wpoints.shape)
    # 如果 wpoints.shape[0] == 4，直接用，不要再加一行
    wpoints_homo = wpoints
    wpoints_world = np.dot(cam2world, wpoints_homo)[:3, :]
    ...
    if max_distance is not None and np.isfinite(max_distance):
        ...
        wpoints_world = wpoints_world[:, depth_mask_keep]  # 注意这里是列向量
        print("imcoords.shape =", imcoords.shape, "depth_mask_keep.shape =", depth_mask_keep.shape)
        imcoords = imcoords[:, depth_mask_keep]
        if colored:
            rgb = rgb[depth_mask_keep, :]

    # 转置为 N×3 格式
    wpoints = np.ascontiguousarray(wpoints_world.T)
