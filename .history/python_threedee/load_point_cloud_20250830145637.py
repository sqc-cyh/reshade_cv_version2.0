...
def build_cam2world_from_json(camjson, transpose_extrinsic=True):
    C = np.float64(camjson['extrinsic_cam2world']).reshape((3,4))
    M = np.eye(4, dtype=np.float64)
    M[:3,:4] = C
    if transpose_extrinsic:
        R = M[:3,:3].T          # 关键：转置旋转
        t = M[:3,3].copy()
        M[:3,:3] = R
        M[:3,3]  = t
...
def load_cloud_via_depth_and_camjson(depthfile:str,
    ...
    cam2world = build_cam2world_from_json(camjson, transpose_extrinsic=True)
