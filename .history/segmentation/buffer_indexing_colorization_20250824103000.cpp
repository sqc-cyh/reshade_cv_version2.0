...
void custom_shader_buffer_visualization_on_reshade_begin_effects(effect_runtime* runtime, command_list* cmd_list, resource_view rtv, resource_view)
{
	device* const device = runtime->get_device();
	auto& mapp = device->get_private_data<segmentation_app_data>();
	// 新帧开始时重置上一帧的 IB/VB 对齐表（避免在 finish_effects 抢先清空）
	...
	reshade::api::command_queue* cmdqueue, simple_packed_buf& segBuf, simple_packed_buf& triBuf, nlohmann::json& dstMetaJson)
{
	...
	char b[160];
    _snprintf_s(b, _TRUNCATE, "[debug] draw_meta=%zu, ibvb=%zu",
                draw_metadata.size(), per_draw_ib_vb.size());
    reshade::log_message(reshade::log_level::info, b);
	if (draw_metadata.empty()) {
	...
	for (auto cmi = color2seg.cbegin(); cmi != color2seg.cend(); ++cmi) {
		idx_color = cmi->first;
		sprintf_s(sprbuf, "%02x%02x%02x",
				idx_color_bytes_view[0], idx_color_bytes_view[1], idx_color_bytes_view[2]);

		// 原 perdraw 元数据（[0]=draw_idx, [1]=hi, [2]=lo）
		const perdraw_metadata_type &meta = cmi->second;
		const uint64_t draw_idx_u64 = meta[0];

		// 从 CPU 侧对齐数组中取 ib/vb0（越界则置 0）
		uint64_t ib  = 0, vb0 = 0;
		if (draw_idx_u64 < per_draw_ib_vb.size()) {
			const auto &p = per_draw_ib_vb[static_cast<size_t>(draw_idx_u64)];
			ib  = p.first;
			vb0 = p.second;
		}

		// 写成五元组：[draw_idx, hi, lo, ib, vb0]
		nlohmann::json arr = nlohmann::json::array();
		arr.push_back(meta[0]);
		arr.push_back(meta[1]);
		arr.push_back(meta[2]);
		arr.push_back(ib);
		arr.push_back(vb0);

		dstMetaJson["seg_hexcolor2meta"][sprbuf] = std::move(arr);
	}
	...
	for (const auto &kv : shader_pix_count){
		...
		oss_key << "0x" << std::uppercase << std::hex << k.s1
				<< "_0x" << k.s2 << std::nouppercase << std::dec;
		std::string key_str = oss_key.str();
