...
static bool g_dump_idmap_every_capture = false;   // 勾选后，每次 CAPTURE 都导出
static bool g_dump_idmap_csv           = true;    // 额外导出 CSV
static char g_dump_dir[260]            = "outputs";  // 导出目录
...
void imgui_draw_custom_shader_debug_viz_in_reshade_overlay(effect_runtime* runtime)
{
	...
	ImGui::Checkbox("Dump Stable-ID JSON/CSV each capture", &g_dump_idmap_every_capture);
	ImGui::SameLine();
	ImGui::Checkbox("CSV too", &g_dump_idmap_csv);
	...
	reshade::api::command_queue* cmdqueue, simple_packed_buf& segBuf, simple_packed_buf& triBuf, nlohmann::json& dstMetaJson)
{
	...
	// 	}
	// }
	// === Optional: dump stable ShaderID -> color & stats (guarded by UI toggle) ===
	if (g_dump_idmap_every_capture)
	{
		// 1) 统计：从 segBuf 的颜色 -> meta（draw_idx, shader_hi, shader_lo）反查
		struct ShaderKey { uint64_t s1, s2; };
		struct KeyHash { size_t operator()(const ShaderKey& k) const noexcept {
			return std::hash<uint64_t>{}(k.s1) ^ (std::hash<uint64_t>{}(k.s2) << 1);
		}};
		...
		try {
			...
			if (g_dump_idmap_csv) {
				std::ofstream cf(std::string(g_dump_dir) + "/id_color_map.csv");
				cf << "shader_id_1,shader_id_2,color_hex,pixel_count\n";
				for (const auto &kv : pixcnt) {
					const ShaderKey &k = kv.first;
					const auto &rgb = repr.at(k);
