...
// === UI 切换：仅控制是否导出稳定ID JSON ===
static bool g_dump_idmap_every_capture = false;
static char g_dump_dir[260]            = "outputs";
...
void imgui_draw_custom_shader_debug_viz_in_reshade_overlay(effect_runtime* runtime)
{
	...
	ImGui::Separator();
	ImGui::Text("Export (on next CAPTURE)");
	ImGui::Checkbox("Dump Stable-ID JSON each capture", &g_dump_idmap_every_capture);
	ImGui::InputText("Output dir", g_dump_dir, IM_ARRAYSIZE(g_dump_dir));
	ImGui::TextDisabled("Files are written when CAPTURE saves seg/tri.");
	...
	reshade::api::command_queue* cmdqueue, simple_packed_buf& segBuf, simple_packed_buf& triBuf, nlohmann::json& dstMetaJson)
{
	...
	if (draw_metadata.empty()) {
		...
		for (uint32_t y = 0; y < tdesc.texture.height; ++y) {
			...
			for (uint32_t x = 0; x < tdesc.texture.width; ++x) {
				...
				// Colorize detailed triangle+metadata map (仅用于 triBuf PNG，可保留)
				...
				if (auto ibo = inst2objid.find(ibuf); ibo != inst2objid.end()) {
					...
					inst2objid.emplace(ibuf, tbuf[std::tuple_size<perdraw_metadata_type>::value] = static_cast<uint32_t>(inst2objid.size()));
				}
				...
				if (auto mci = tri2color.find(tbuf); mci != tri2color.end()) {
					...
					if (color2tri.count(idx_color)) reshade::log_message(reshade::log_level::error, "error: repeated color in colormap");
	...
	// 写入 seg 的颜色->元数据；不再写 tri_hexcolor2meta
	char sprbuf[16];
	for (auto cmi = color2seg.cbegin(); cmi != color2seg.cend(); ++cmi) {
		idx_color = cmi->first;
		sprintf_s(sprbuf, "%02x%02x%02x", reinterpret_cast<uint8_t*>(&idx_color)[0],
		                                 reinterpret_cast<uint8_t*>(&idx_color)[1],
		                                 reinterpret_cast<uint8_t*>(&idx_color)[2]);
		dstMetaJson["seg_hexcolor2meta"][sprbuf] = cmi->second;
	}
	// —— 删除了对 color2tri 的 JSON 输出 —— //

	// 可选：导出“稳定 ShaderID -> 代表色&像素数”的 JSON
	if (g_dump_idmap_every_capture)
	{
		...
		std::unordered_map<ShaderKey, uint64_t, KeyHash, KeyEq> pixcnt;
		std::unordered_map<ShaderKey, std::array<uint8_t,3>, KeyHash, KeyEq> repr;
		...
		const uint32_t W = segBuf.width, H = segBuf.height;
		for (uint32_t y = 0; y < H; ++y) {
			for (uint32_t x = 0; x < W; ++x) {
				...
				const auto &meta = it.value();   // [draw_idx, shader_hi, shader_lo]
				ShaderKey key{ (uint64_t)meta[1], (uint64_t)meta[2] };
				pixcnt[key] += 1;
				if (!repr.count(key)) repr[key] = { p[0], p[1], p[2] };
			}
		}

		nlohmann::json j_id_map;
		for (const auto &kv : pixcnt) {
			const ShaderKey &k = kv.first;
			const auto &rgb = repr.at(k);
			const uint32_t rgb24 = (uint32_t)rgb[0] | ((uint32_t)rgb[1] << 8) | ((uint32_t)rgb[2] << 16);
			std::ostringstream oss; oss << "0x" << std::hex << k.s1 << "_0x" << std::hex << k.s2;
			j_id_map[oss.str()] = { {"color_hex", hex_of_rgb(rgb24)}, {"pixel_count", kv.second} };
		}
		...
		// 不再包含 tri_hexcolor2meta
		j_out["stable_id_to_color"] = std::move(j_id_map);

		try {
			std::filesystem::create_directories(g_dump_dir);
			std::ofstream jf(std::string(g_dump_dir) + "/seg_map.json", std::ios::binary);
			jf << j_out.dump(2);
			reshade::log_message(reshade::log_level::info, "[seg] dumped stable-id map");
		} catch (...) {
			reshade::log_message(reshade::log_level::warning, "[seg] dump stable-id map failed");
		}
	}
