...
static bool is_glassmask_enabled(reshade::api::effect_runtime* runtime) {
    using namespace reshade::api;
    const char* fx = "glassmask.fx";
    const char* tech = "GlassMask";
    effect_technique t = runtime->find_technique(fx, tech);
    if (t.handle == 0ull) return false;
    return runtime->get_technique_state(t);
}

static void on_device_init(reshade::api::device* device) {
    auto &mapp = device->create_private_data<segmentation_app_data>(); // [保留] 你的原始逻辑

    // === [ADD] 默认路径：<游戏目录>\glass_ids.json ===
    try {
        std::filesystem::path glass_path = get_game_dir() / "glass_ids.json";
        if (std::filesystem::exists(glass_path)) {
            if (mapp.load_glass_ids_json(glass_path.string())) {
                reshade::log_message(reshade::log_level::info,
                    (std::string("Loaded glass_ids.json from ") + glass_path.string()).c_str());
            } else {
                reshade::log_message(reshade::log_level::warning,
                    (std::string("Failed to parse glass_ids.json at ") + glass_path.string()).c_str());
            }
        } else {
            reshade::log_message(reshade::log_level::info,
                (std::string("glass_ids.json not found at ") + glass_path.string()).c_str());
        }
    } catch (...) {
        reshade::log_message(reshade::log_level::error, "Exception when loading glass_ids.json");
    }
}
...
bool segmapp_on_draw_plain_or_indexed(reshade::api::command_list* cmd_list, uint32_t vertices_per_instance, uint32_t instance_count, uint32_t first_index, int32_t vertex_offset, uint32_t first_instance) {
	auto& cmdlst_state = cmd_list->get_private_data<segmentation_app_cmdlist_state>();
	{
		...
		if (mapp.glassmask_on && !mapp.glass_ids.empty() &&
			...
			// segmentation 管线必须处于“可拦截”状态（与可视化同一套安全约束）
			mapp.do_intercept_draw &&
			mapp.r_accum_bonus.is_valid() &&
			mapp.r_counter_buf.is_valid())
		{
			// 仅在“当前绑定的 RT/Depth == 本帧呈现目标”时才允许拦截，避免误吞后处理/合成
			...
			{
				// 跳过疑似全屏清屏/合成 pass（避免黑屏）
				const bool is_fullscreen_like = (vertices_per_instance <= 6 && instance_count <= 1);
				if (!is_fullscreen_like)
				{
					// 绝不拦截 segmentation 自己的“bonus RTV 重绘”——
					// 当 bonus RTV 已绑定时，当前 RT 资源会是 r_accum_bonus.rsc
					const bool drawing_to_bonus_rtv =
						(curr_rt_res == mapp.r_accum_bonus.rsc);
					if (!drawing_to_bonus_rtv)
					{
						// 仅取 ShaderID 做匹配；命中才拦截
						perdraw_metadata_type meta =
							get_draw_metadata_including_shader_info(device, cmd_list, vertices_per_instance);
						const uint64_t hi = meta[1], lo = meta[2];

						if (mapp.glass_ids.find({hi, lo}) != mapp.glass_ids.end())
						{
							// ✅ 满足所有安全条件且在名单内：吞掉该 drawcall
							return true;
						}
					}
				}
    ...
    if (j.contains("ShaderIDs")) {
        for (auto &s : j["ShaderIDs"]) {
            if (s.is_string()) {
                ...
                uint64_t hi = 0, lo = 0;
                try {
                    hi = std::stoull(hi_s, nullptr, 16);
                    lo = std::stoull(lo_s, nullptr, 16);
                } catch (...) {
                    try { // 兼容十进制
                        hi = std::stoull(hi_s, nullptr, 10);
                        lo = std::stoull(lo_s, nullptr, 10);
                    } catch (...) { continue; }
                }
                glass_ids.emplace(hi, lo);
            } else if (s.is_array() && s.size() == 2) {
                try {
                    uint64_t hi = s[0].get<uint64_t>();
                    uint64_t lo = s[1].get<uint64_t>();
                    glass_ids.emplace(hi, lo);
                } catch (...) { /* ignore */ }
            }
        }
