...
bool segmapp_on_draw_plain_or_indexed(reshade::api::command_list* cmd_list, uint32_t vertices_per_instance, uint32_t instance_count, uint32_t first_index, int32_t vertex_offset, uint32_t first_instance) {
	auto& cmdlst_state = cmd_list->get_private_data<segmentation_app_cmdlist_state>();
	{
		...
		// 仅当 glassmask 开启、且与可视化路径相同的绑定/资源条件满足时，才考虑拦截
		if (mapp.glassmask_on
			...
			if (mapp.rsrcs_of_presented_rgb.count(currst0rsrc.handle)
				&& currdepthrc == mapp.rsrc_of_presented_depth) {

				// 跳过全屏清屏/后处理的疑似 pass，避免黑屏
				const bool is_fullscreen_draw = (vertices_per_instance <= 6 && instance_count <= 1);
				if (!is_fullscreen_draw) {
					// 仅获取本次 draw 的 ShaderID，命中则吞掉
					perdraw_metadata_type meta = get_draw_metadata_including_shader_info(
						device, cmd_list, vertices_per_instance);
					const uint64_t shader_hi = meta[1];
					const uint64_t shader_lo = meta[2];

					if (mapp.glass_ids.find({shader_hi, shader_lo}) != mapp.glass_ids.end()) {
						return true; // 吞掉该 drawcall；其它 draw 与 ReShade 合成不受影响
					}
				}
			}
		}
