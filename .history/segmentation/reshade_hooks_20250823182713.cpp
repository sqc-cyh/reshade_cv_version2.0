...
#endif
#include <atomic>
static std::atomic_bool g_has_logged_this_frame = false;
...
bool segmapp_on_draw_plain_or_indexed(reshade::api::command_list* cmd_list, uint32_t vertices_per_instance, uint32_t instance_count, uint32_t first_index, int32_t vertex_offset, uint32_t first_instance) {
	auto& cmdlst_state = cmd_list->get_private_data<segmentation_app_cmdlist_state>();
	{
		...
		auto& mapp   = device->get_private_data<segmentation_app_data>();
		if (!g_has_logged_this_frame.exchange(true)) // 确保这个代码块每帧只执行一次
		{
			char log_buffer[256];
			snprintf(log_buffer, sizeof(log_buffer),
				"[GlassMask Debug] Frame Start. glassmask_on: %d, do_intercept_draw: %d, glass_ids_loaded: %zu",
				mapp.glassmask_on,
				mapp.do_intercept_draw,
				mapp.glass_ids.size()
			);
			reshade::log_message(reshade::log_level::info, log_buffer);
		}
			...
			if (on_present_targets) {
				...
				if (!is_fullscreen_like) {
					...
					if (!drawing_to_bonus_rtv) {
						...
						if (mapp.glass_ids.find({hi, lo}) != mapp.glass_ids.end()) {
							return true; // ✅ 命中 JSON 的玻璃 → 吞掉该 drawcall
						}
					}
				}
			}
