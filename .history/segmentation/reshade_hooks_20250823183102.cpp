...
bool segmapp_on_draw_plain_or_indexed(reshade::api::command_list* cmd_list, uint32_t vertices_per_instance, uint32_t instance_count, uint32_t first_index, int32_t vertex_offset, uint32_t first_instance) {
	auto& cmdlst_state = cmd_list->get_private_data<segmentation_app_cmdlist_state>();
	{
		...
		if (mapp.glassmask_on && !mapp.glass_ids.empty() && vertices_per_instance > 2)
		{
			if (!cmdlst_state.rtvs.empty() && cmdlst_state.rtvs[0].handle != 0ull && cmdlst_state.dsv.handle != 0ull)
			{
				...
				if (on_present_targets && !is_fullscreen_like)
				{
					...
					{
						// --- BEGIN: 修改拦截成功日志 ---
						// 现在它会打印每一个被拦截的 draw call 的信息
						char log_buffer[256];
						snprintf(log_buffer, sizeof(log_buffer), 
							"[GlassMask Intercept] VS: %llu, PS: %llu, Vertices: %u", 
							vs_hash, ps_hash, vertices_per_instance);
						reshade::log_message(reshade::log_level::info, log_buffer);
						// --- END: 修改拦截成功日志 ---
						return true;
					}
