...
bool segmapp_on_draw_plain_or_indexed(reshade::api::command_list* cmd_list, uint32_t vertices_per_instance, uint32_t instance_count, uint32_t first_index, int32_t vertex_offset, uint32_t first_instance) {
    ...
    // --- 分支 1: Segmentation 捕获逻辑 ---
    // 仅当 do_intercept_draw 为 true 时，才进入此分支。
    // 这是重量级操作，会重定向渲染并可能返回 true。
    if (mapp.do_intercept_draw)
    {
        if (mapp.r_accum_bonus.is_valid() && mapp.r_counter_buf.is_valid() &&
            vertices_per_instance > 1 && !cmdlst_state.rtvs.empty() && cmdlst_state.rtvs[0].handle != 0ull && cmdlst_state.dsv.handle != 0ull)
        {
            ...
            if (mapp.rsrcs_of_presented_rgb.count(currst0rsrc.handle) && currdepthrc == mapp.rsrc_of_presented_depth) {
                const bool is_fullscreen_draw = (vertices_per_instance <= 6 && instance_count <= 1);
    ...
    // --- 分支 2: 玻璃拦截逻辑 ---
    // 仅当 do_intercept_draw 为 false 且 glassmask_on 为 true 时，才考虑进入此分支。
    // 这是轻量级操作，只检查并可能返回 true。
    else if (mapp.glassmask_on && !mapp.glass_ids.empty())
    {
        // 安全检查
        if (vertices_per_instance > 2 && !cmdlst_state.rtvs.empty() && cmdlst_state.rtvs[0].handle != 0ull && cmdlst_state.dsv.handle != 0ull)
        {
            ...
            const bool on_present_targets = mapp.rsrcs_of_presented_rgb.count(curr_rt_res.handle) && (curr_ds_res == mapp.rsrc_of_presented_depth);
            const bool is_fullscreen_like = (vertices_per_instance <= 6 && instance_count <= 1);

            if (on_present_targets && !is_fullscreen_like)
            {
                perdraw_metadata_type meta = get_draw_metadata_including_shader_info(device, cmd_list, vertices_per_instance);
                const uint64_t vs_hash = meta[1];
                const uint64_t ps_hash = meta[2];

                if (mapp.glass_ids.count({vs_hash, ps_hash}))
                {
                    return true; // ✅ 拦截成功，玻璃被移除
                }
            }
        }
    }
    ...
    // 如果以上两个分支的条件都不满足，或者分支内部的条件不满足，则默认不拦截。
    return false;
}
