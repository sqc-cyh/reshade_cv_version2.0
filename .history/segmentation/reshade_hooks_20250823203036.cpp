...
bool segmapp_on_draw_plain_or_indexed(reshade::api::command_list* cmd_list, uint32_t vertices_per_instance, uint32_t instance_count, uint32_t first_index, int32_t vertex_offset, uint32_t first_instance) {
    ...
    if (mapp.glassmask_on &&                // 只在开关开启时观察
        !cmdlst_state.rtvs.empty() &&
        cmdlst_state.rtvs[0].handle != 0ull &&
        cmdlst_state.dsv.handle    != 0ull)
    {
        const auto rt = device->get_resource_from_view(cmdlst_state.rtvs[0]);
        const auto ds = device->get_resource_from_view(cmdlst_state.dsv);

        const bool on_present =
            mapp.rsrcs_of_presented_rgb.count(rt.handle) &&
            (ds == mapp.rsrc_of_presented_depth);
        ...
        if (on_present) {
            ...
            // 取像素着色器哈希
            uint64_t ps_hash = 0;
            auto itps = mapp.pipeline_handle_to_pixel_shader_hash.find(
                cmdlst_state.pipelines[reshade::api::pipeline_stage::pixel_shader].handle);
            if (itps != mapp.pipeline_handle_to_pixel_shader_hash.end())
                ps_hash = itps->second;

            // 取 blend 状态（只对开启混合的全屏 pass 感兴趣）
            reshade::api::pipeline gp = cmdlst_state.pipelines[reshade::api::pipeline_stage::all];
            if (gp.handle == 0ull)
                gp = cmdlst_state.pipelines[reshade::api::pipeline_stage::pixel_shader];

            bool alpha_blend = false;
            if (gp.handle != 0ull) {
                reshade::api::pipeline_desc pdesc{};
                if (device->get_pipeline_desc(gp, &pdesc)) {
                    for (int i = 0; i < 8; ++i)
                        if (pdesc.blend_state.targets[i].blend_enable) { alpha_blend = true; break; }
                }
            }

            static uint32_t gate = 0;
            if (is_fullscreen_like && alpha_blend && ps_hash != 0 && ((++gate % 60u) == 1u)) {
                char buf[128];
                _snprintf_s(buf, _TRUNCATE, "Glass candidate (fullscreen+blend) PSHash=0x%016llX",
                            (unsigned long long)ps_hash);
                reshade::log_message(reshade::log_level::info, buf);
            }
        }
    }
}
