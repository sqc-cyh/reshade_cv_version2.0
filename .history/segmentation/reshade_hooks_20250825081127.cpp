...
static bool is_glassmask_enabled(reshade::api::effect_runtime* rt) {
    ...
    if (auto t = rt->find_technique("objectmask.fx", "ObjectMask"); t.handle != 0ull)
        return rt->get_technique_state(t);
    return false;
}

bool segmentation_app_data::load_mask_ids_json(const std::string& path) {
    std::ifstream f(path);
    if (!f) return false;
    ...
    if (j.contains("ShaderIDs")) {
        for (auto &s : j["ShaderIDs"]) {
            if (s.is_string()) {
                ...
                mask_ids.emplace(hi, lo);
            } else if (s.is_array() && s.size() == 2) {
                try { mask_ids.emplace(s[0].get<uint64_t>(), s[1].get<uint64_t>()); } catch (...) {}
            }
        }
    }
    ...
    return !mask_ids.empty();
}

static void on_device_init(reshade::api::device* device) {
    auto& mapp = device->create_private_data<segmentation_app_data>();

    // 从 <游戏.exe 同目录>\mask_ids.json 读取白名单（存在时加载）
    try {
        const auto p = get_game_dir() / "mask_ids.json";
        if (std::filesystem::exists(p)) {
            if (mapp.load_mask_ids_json(p.string())) {
                reshade::log_message(reshade::log_level::info,
                    (std::string("Loaded mask_ids.json from ") + p.string()).c_str());
            } else {
                reshade::log_message(reshade::log_level::warning,
                    (std::string("Failed to parse mask_ids.json at ") + p.string()).c_str());
            }
        }
    } catch (...) {
        reshade::log_message(reshade::log_level::warning, "Exception when loading mask_ids.json");
    }
}
...
bool segmapp_on_draw_plain_or_indexed(reshade::api::command_list* cmd_list, uint32_t vertices_per_instance, uint32_t instance_count, uint32_t first_index, int32_t vertex_offset, uint32_t first_instance) {
	auto& cmdlst_state = cmd_list->get_private_data<segmentation_app_cmdlist_state>();
	{
		...
		if (mapp.glassmask_on && !mapp.mask_ids.empty()
			...
			if (on_present_targets) {
				...
				if (!is_fullscreen_like) {
					...
					if (!drawing_to_bonus_rtv) {
						...
						const bool id_hit = (mapp.mask_ids.find({hi, lo}) != mapp.mask_ids.end());
