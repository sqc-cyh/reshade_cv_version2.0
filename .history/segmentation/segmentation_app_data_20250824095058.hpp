...
struct __declspec(uuid("bab8ffb9-5e8b-4b48-b4f2-4bac4bf87e82")) segmentation_app_data : public segmentation_app_buffer_indexing_colorization {
	...
	bool glassmask_on = false;           // 每帧刷新：是否启用 glassmask.fx::GlassMask
	bool segmentation_viz_on = false;    // 每帧刷新：SemSegView 是否启用（用来确保不影响 segmentation）
	...
	std::unordered_set<std::pair<uint64_t,uint64_t>, PairHash> glass_ids;
	std::unordered_set<uint64_t> glass_ps_hashes;

	// 从磁盘加载 EXE 同目录的 glass_ids.json
	bool load_glass_ids_json(const std::string& path);
	std::vector<std::pair<uint64_t, uint64_t>> per_draw_ib_vb; // 顺序与 stash_metadata 调用顺序一致

	// 帧末清空（如果你已有 reset_at_end_of_frame，可叠加；否则第 4 步会在 frame end 清空）
	inline void per_draw_ib_vb_reset() { per_draw_ib_vb.clear(); }
};
