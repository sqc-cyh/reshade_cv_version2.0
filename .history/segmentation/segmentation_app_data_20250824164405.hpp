...
struct __declspec(uuid("bab8ffb9-5e8b-4b48-b4f2-4bac4bf87e82")) segmentation_app_data : public segmentation_app_buffer_indexing_colorization {
	...
	bool glassmask_on = false;           // 是否启用 glassmask.fx::ObjectMask
	...
	std::unordered_set<std::pair<uint64_t,uint64_t>, PairHash> mask_ids;
	std::unordered_set<uint64_t> glass_ps_hashes;

	// 从磁盘加载 EXE 同目录的 mask_ids.json
	bool load_mask_ids_json(const std::string& path);
};
