...
#include "AssassinsCreedOdyssey.h"
#include "gcv_utils/depth_utils.h"
#include "gcv_utils/scripted_cam_buf_templates.h"


std::string GameAssassinsCreedOdyssey::gamename_verbose() const { return "AssassinsCreedOdyssey"; } // hopefully continues to work with future patches via the mod lua

std::string GameAssassinsCreedOdyssey::camera_dll_name() const { return ""; } // no dll name, it's available in the exe memory space
uint64_t GameAssassinsCreedOdyssey::camera_dll_mem_start() const { return 0; }
GameCamDLLMatrixType GameAssassinsCreedOdyssey::camera_dll_matrix_format() const { return GameCamDLLMatrix_allmemscanrequiredtofindscriptedcambuf; }

scriptedcam_checkbuf_funptr GameAssassinsCreedOdyssey::get_scriptedcambuf_checkfun() const {
	return template_check_scriptedcambuf_hash<double, 13, 1>;
}
uint64_t GameAssassinsCreedOdyssey::get_scriptedcambuf_sizebytes() const {
	return template_scriptedcambuf_sizebytes<double, 13, 1>();
}
bool GameAssassinsCreedOdyssey::copy_scriptedcambuf_to_matrix(uint8_t* buf, uint64_t buflen, CamMatrixData& rcam, std::string& errstr) const {
	return template_copy_scriptedcambuf_extrinsic_cam2world_and_fov<double, 13, 1>(buf, buflen, rcam, true, errstr);
}

bool GameAssassinsCreedOdyssey::can_interpret_depth_buffer() const {
	return true;
}
float GameAssassinsCreedOdyssey::convert_to_physical_distance_depth_u64(uint64_t depthval) const {
...
uint64_t GameAssassinsCreedOdyssey::get_scriptedcambuf_triggerbytes() const
{
...
void GameAssassinsCreedOdyssey::process_camera_buffer_from_igcs(
